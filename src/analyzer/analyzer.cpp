...
void CheckReturnTypes::check_tree() {
...
std::shared_ptr<parsing::Type> CheckReturnTypes::convert(std::string& type) {
...
void CheckReturnTypes::write_out_types(std::vector<std::shared_ptr<parsing::ASTNode>>& nodes) {
    for (auto& code_node : nodes) {
        if (code_node->get_grammar() == GrammarUnit::VARIABLE) {
            ...
            m_var_map[variable->m_name] = convert(variable->m_type->m_name);

            if (variable->m_assigned.get()) {
                // Check for correctness of assignments
                std::cout << "Checking initializing of " + variable->m_name + " (" + variable->m_type->m_name + ")...\n";
                auto result_type = get_expression_type(variable->m_assigned);
                result_type->print();

                if (result_type->m_name != variable->m_type->m_name) {
                    if (!is_primitive(result_type->m_name) || !is_primitive(variable->m_type->m_name)) {
                        print_error("Invalid initializing of " + variable->m_name + " (" + variable->m_type->m_name + ")");
                    }
                }
...
std::shared_ptr<parsing::Type> CheckReturnTypes::get_expression_type(std::shared_ptr<parsing::Expression> &expression) {
    switch (expression->get_grammar()) {
        ...
        case GrammarUnit::IDENTIFIER: {
            ...
            auto cur_type = m_var_map[variable->m_head_name];
